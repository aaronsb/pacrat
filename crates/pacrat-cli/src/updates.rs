...
/// the same remotes and must ask it the same guarded way. The guards — the
/// `--`, the closed interactive doors, the timeout — and the reasons for
/// them live in [`crate::git`]; the argv line goes wherever this run's
/// chatter goes ([`crate::out`]).
