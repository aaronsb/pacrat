...
/// One git invocation: its argv and its bound.
///
/// Where the argv line *goes* is not a property of the call: it is a
/// property of the run, and `crate::out` holds it. stdout for a human,
/// stderr when stdout belongs to a machine-readable answer — `pacrat updates
/// --format json` still shows every call it makes, because "external calls
/// are always visible" has no quiet mode; the format changes where the trace
/// lands, never whether it exists. Threading that through every call site as
/// an argument meant every future caller had to know to pass it, and one
/// that forgot would corrupt a JSON answer rather than fail.
pub struct Git {
    argv: Vec<OsString>,
    timeout: Duration,
}
...
impl Git {
    /// A local call; [`Git::timeout`] is the override for one that reaches
    /// the network.
